...
int buttonPins[NUMOFPINS] = {LEFT_PIN, RIGHT_PIN, UP_PIN, DOWN_PIN, BUTTON1_PIN, BUTTON2_PIN, BUTTON3_PIN, BUTTON4_PIN, BUTTON5_PIN};
...
// //   attachInterrupt(BUTTON3_PIN, button3_ISR, CHANGE);
// //   attachInterrupt(BUTTON4_PIN, button4_ISR, CHANGE);
// //   attachInterrupt(BUTTON5_PIN, button5_ISR, CHANGE);
...
uint32_t buttons, buttonsLast, x, y, xLast, yLast;
bool stateChanged;

void loop()
{
  /* scan GPIO, since each report can has up to 6 keys
   * we can just assign a slot in t he report for each GPIO 
   */
  ...
  if(buttonsLast != buttons || xLast != x || yLast != y){ stateChanged = true;}
